  ...
  async function handleSubmit(e) {
    ...
    try {
      if (esEdicion) {
        ...
        const actualizado = { ...itemEditar, nombre: form.nombre, categoriaId: form.categoriaId, estado: form.estado, puntuacion: form.puntuacion !== '' ? Number(form.puntuacion) : null, notas: form.notas, atributos, fechaActividad };
        await guardarItem(actualizado, { esNuevo: false });
        dispatch({ type: 'REGISTRAR_ACTIVIDAD', payload: { tipo: 'editar', id: itemEditar.id, fecha: fechaActividad } });
        onCancelar(); // señal para que App recargue
      } else {
        const nuevo = crearItem({ ...form, atributos });
        const guardado = await guardarItem(nuevo, { esNuevo: true });
        dispatch({ type: 'AGREGAR',             payload: guardado });
        dispatch({ type: 'REGISTRAR_ACTIVIDAD', payload: { tipo: 'agregar', id: guardado.id, fecha: guardado.fechaActividad } });
